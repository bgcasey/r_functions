calculate_focal_stat <-
  ...
  function(raster_input, window_size_meters, fun, n_cores = 1) {
    ...
    split_raster <- function(raster, n_chunks) {
      ...
      for (i in 1:n_chunks) {
        ...
        chunks[[i]] <- crop(raster, rowFromY(raster, start_row), rowFromY(raster, end_row))
      }
